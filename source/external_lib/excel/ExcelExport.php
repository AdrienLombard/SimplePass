...
class ExcelExport
{
   ...
   */
  public function __construct($columns = null, $culture = null, $rotate = false)
  {
...
<table  style="border-top: 1px solid #AAA; border-left: 1px solid #AAA;">';
    ...
    {
      $iRotate = ($this->rotate)? 'mso-rotate: 70; height: 180px; text-align: left;' : '';
		$excel_contents .= sprintf('<td style="' . $iRotate . ' border-bottom: 1px solid #AAA; border-right: 1px solid #AAA; font-weight: bold;">&nbsp;%s</th>', nl2br(htmlspecialchars($this->getColumnTitle($column_code))));
    ...
    foreach($contents as $content_line) // For each content line.
    {
      ...
      foreach($column_codes as $column_code) // For each column code.
      {
        $excel_contents .= sprintf('<td style="border-bottom: 1px solid #AAA; border-right: 1px solid #AAA;">%s</td>', $this->formatColumnValue($column_code, $content_line[$column_code]));
      } // For each column code.
      $excel_contents .= '</tr>';
    } // For each content line.
